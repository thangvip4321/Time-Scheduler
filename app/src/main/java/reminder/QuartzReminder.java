...
public class QuartzReminder implements Reminder {
    ...
    @Override
    public void sendMailBefore5Min( Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minusSeconds(5*60);
        sendNotification(event.eventID, schedTime);

    }

    @Override
    public void sendMailBefore10Min( Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(10, ChronoUnit.MINUTES);
        sendNotification(event.eventID, schedTime);
    }

    @Override
    public void sendMailBefore15Min( Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(15, ChronoUnit.MINUTES);
        sendNotification(event.eventID, schedTime);
    }

    @Override
    public void sendMailBefore30Min(Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(30, ChronoUnit.MINUTES);
        sendNotification(event.eventID, schedTime);
    }

    @Override
    public void sendMailBefore1Hour(Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(1, ChronoUnit.HOURS);
        sendNotification(event.eventID, schedTime);
    }
    ...
    public void sendMail(int eid) {
        ...
        try {
            ...
            JobDetail jobDetail = buildJobDetail(eid);
            Cron cron = CronBuilder(e.startFrom);
            Trigger trigger = buildJobTrigger(jobDetail, cron);
    ...
    @Override
    public void sendMailBefore3Days(Event event) throws Exception {

        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(3, ChronoUnit.DAYS);
        sendNotification(event.eventID, schedTime);
    }

    @Override
    public void sendMailBefore1Week(Event event) throws Exception {
        
        logger.info("preparing to fire alarm for reminding clients");
        Instant schedTime = Instant.from(event.startFrom).minus(1, ChronoUnit.WEEKS);
        sendNotification(event.eventID, schedTime);
    }
